...
const CVUpload: React.FC<CVUploadProps> = ({ onUsersCreated }) => {
  ...
  const [parsedData, setParsedData] = useState<ParsedCVData[]>([]);
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.type === 'application/zip' || file.name.toLowerCase().endsWith('.zip')) {
        ...
        showNotification('ZIP file selected. Click "Process CVs" to extract and parse.', 'info');
      } else {
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    if (files.length > 0) {
      ...
      if (file.type === 'application/zip' || file.name.toLowerCase().endsWith('.zip')) {
        setUploadedFile(file);
        showNotification('ZIP file dropped. Click "Process CVs" to extract and parse.', 'info');
      } else {
  ...
  const createUserAccounts = async () => {
    if (selectedCVs.size === 0) {
      showNotification('Please select at least one CV to create user accounts.', 'warning');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.CV_UPLOAD}/create-users`, {
        ...
        body: JSON.stringify({ cvData: selectedData })
      });
      ...
      showNotification(
        `Successfully created ${result.created} user accounts!`,
        'success'
      );

      // Call callback if provided
      if (onUsersCreated && result.results) {
        onUsersCreated(result.results);
      }
      ...
      showNotification(
        error instanceof Error ? error.message : 'Failed to create user accounts',
        'error'
      );
    } finally {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <h2 style={{ marginBottom: '1rem' }}>📁 Bulk CV Upload & AI Parsing</h2>
        <p style={{ color: 'var(--text-secondary)', maxWidth: '600px', margin: '0 auto' }}>
          Upload a ZIP file containing CV documents (PDF, DOCX, DOC). Our AI will automatically extract 
          key information and create user accounts for your recruitment platform.
        </p>
      </div>
      ...
      {!showResults ? (
        <div style={{ 
          ...
          <div style={{ 
            ...
            <ol style={{ margin: 0, paddingLeft: '1.5rem' }}>
              ...
              <li>Click "Process CVs" to extract and parse information</li>
              <li>Review the extracted data and select which CVs to create accounts for</li>
              <li>Click "Create User Accounts" to register the candidates</li>
            </ol>
          </div>
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: '2rem' }}>
            <p>
              <strong>{selectedCVs.size}</strong> of <strong>{parsedData.length}</strong> CVs selected for account creation
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
            <button
              ...
              disabled={selectedCVs.size === 0 || isCreatingUsers}
              style={{
                padding: '1rem 2rem',
                background: selectedCVs.size > 0 && !isCreatingUsers ? 'var(--success)' : 'var(--text-secondary)',
                color: 'white',
                border: 'none',
                borderRadius: 'var(--radius)',
                cursor: selectedCVs.size > 0 && !isCreatingUsers ? 'pointer' : 'not-allowed',
                fontWeight: 600,
                fontSize: '1rem'
              }}
            >
              {isCreatingUsers ? (
                ...
                `👥 Create ${selectedCVs.size} User Account${selectedCVs.size !== 1 ? 's' : ''}`
              )}
            </button>
          </div>
        </div>
